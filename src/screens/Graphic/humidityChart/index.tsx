...
const HumidityChart: React.FC<HumidityChartProps> = ({ startDate, endDate }) => {
  ...
  return (
    <View style={Styles.container}>
      <Text style={Styles.title}>Gráfico de Umidade</Text>
      {data.length > 0 ? (
        <LineChart
          data={{
            labels: data.map((item) => new Date(item._id).getDate().toString()), // Ajuste aqui
            datasets: [{ data: data.map((item) => item.avgHumidity) }],
          }}
          width={400}
          height={200}
          yAxisSuffix="%"
